...
$Elements = [
    [
        '37fh29d',    // unique id of this element (i.e. its key)
            /**
             * Instead of using the array key as the id (and in Climber), here we're using the first item
             * in the brick array as the ID. This means that nearly all operations querying against bricks
             * should use `array_column` or something similar. i.e. `array_column($array, 1, 0)`, which would
             * return an array of parents, indexed by slot 0.
             */
        3,          // ID of parent; `null` if no parent
            /**
             * Bricks will not store their own children: Storing children makes moving bricks much more complicated
             * and prone to problems, because values must be changed on parent *and* child bricks. If only the parent
             * is stored, then making changes is much more trivial.
             * Thought should be given re: how to optimize calculations for who is the child of what, etc.
             */
        'div',      // Element type
        [['class'], ['another_class', '__', 'prepend-this']], // Classes
            /**
             * Class is an array of arrays. Each secondary array is only *required* to have a single item;
             * This item is the "base" class. All following items will be concatenated in reverse order. In
             * other words, the array item ['base', '__', 'pre'] will come out 'pre__base'. Additionally,
             * while the base spec does not use keys, it should concatenate the array in such a was as to allow
             * for their use.
             */
    ]
];
